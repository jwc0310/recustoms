...
public class MultiDirectionSlidingDrawer extends ViewGroup implements View.OnClickListener {
	...
	private boolean						mVertical;		/** 是否是垂直方向 **/
	...
	public void showAnim(){
		mHandle.startAnimation(swingAnimation);
	}
	...
	protected void onFinishInflate() {
		...
		mContent.findViewById(R.id.xy_back_arrow).setOnClickListener(this);
		downAndUp = (TextView) mContent.findViewById(R.id.xy_up_down_arrow);
		downAndUp.setOnClickListener(this);

		swingAnimation = new SwingAnimation(
				0f, 15f, -15f, Animation.RELATIVE_TO_SELF, 0.5f, Animation.RELATIVE_TO_SELF, 0.0f);
		swingAnimation.setDuration(4000);     //鍔ㄧ敾鎸佺画鏃堕棿
		swingAnimation.setRepeatCount(0);     //鍔ㄧ敾閲嶆挱娆℃暟
		swingAnimation.setFillAfter(true);  //鏄惁淇濇寔鍔ㄧ敾缁撴潫鐢婚潰
		swingAnimation.setStartOffset(500);   //鍔ㄧ敾鎾斁寤惰繜
	}

	private TextView downAndUp;
	...
	public static final String	LOG_TAG	= "Andy";
		...
		if ( mTracking ) {
			...
			switch ( action ) {
				...
				case MotionEvent.ACTION_MOVE:
					int tmp = (int)(mVertical ? event.getY() : event.getX()) - mTouchDelta;
					moveHandle(tmp < 0 ? 0 : tmp);
					break;
...
//		if ( content.isLayoutRequested() ) {
		if ( true ) {
			if ( mVertical ) {
				...
				int height;
				if (mExpanded){
					height = getBottom() - getTop() - mTopOffset;
				}else {
					height = getBottom() - getTop() - handleHeight - mTopOffset;
				}
